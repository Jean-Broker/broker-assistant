// --- الدوال الأساسية ومربوطة بـ Window عشان متفصلش أبداً ---
...
// --- Theme & Setup ---
window.setNavForApp = function(isAppView) { const links = document.getElementById('siteBarLinks'), loginBtn = document.getElementById('siteBarLoginBtn'); if (links) links.classList.toggle('nav-app-hidden', isAppView); if (loginBtn) loginBtn.classList.toggle('nav-app-hidden', isAppView); };
if (sessionStorage.getItem('isSystemOpen') === 'true') { document.getElementById('landingPageContainer').style.display = 'none'; document.getElementById('systemApp').style.display = 'flex'; window.setNavForApp(true); } else { document.getElementById('landingPageContainer').style.display = 'block'; document.getElementById('systemApp').style.display = 'none'; window.setNavForApp(false); }
...
// 🚨 زراير الدخول اللي فصلت رجعتلها الـ window عشان تشتغل 100% 🚨
...
window.backToLanding = function() { document.getElementById('systemApp').style.display = 'none'; document.getElementById('landingPageContainer').style.display = 'block'; window.setNavForApp(false); };
window.handleAuthAction = function() { currentUser ? (auth.signOut(), sessionStorage.removeItem('isSystemOpen'), window.backToLanding()) : document.getElementById('paywallModal').style.display = 'flex'; };
...
// 🚨 حماية الـ Firebase عشان ميعملش Crash ويفصل الصفحة 🚨
...
document.addEventListener('keydown', (e) => {
    ...
    if (document.getElementById('filterDrawer') && document.getElementById('filterDrawer').classList.contains('open')) window.closeFilterDrawer();
    if (document.getElementById('paywallModal').style.display === 'flex') window.closeLoginModal();
...
window.toggleDropdown = function(id) { const wrapper = document.getElementById(id).parentElement; const isActive = wrapper.classList.contains('active'); window.closeAllDropdowns(); if (!isActive) wrapper.classList.add('active'); };
...
document.addEventListener('click', function(event) { if (!event.target.closest('.filter-dropdown-wrapper') && !event.target.closest('.glass-search-container') && !event.target.closest('.filter-drawer')) { window.closeAllDropdowns(); } });
...
window.setCompletionFilter = function(filterType, btnElem) { completionFilter = filterType; document.querySelectorAll('.stat-card').forEach(c => c.classList.remove('active')); btnElem.classList.add('active'); window.renderGrid(); };
...
window.saveMainLocationsToCloud = async function() { if(isEditor) { try { await db.collection('system').doc('locations').set({ mainLocations }); } catch (error) {} } };
  ...
  try {
      ...
      if(fldLoc && Array.isArray(mainLocations)) {
          fldLoc.innerHTML = `<option value="">-- لم يتم تحديد فرع --</option>` + mainLocations.map(m => {
              if(!m) return '';
              let subOptions = Array.isArray(m.subLocations) ? m.subLocations.map(s => `<option value="${s.id}">${window.escapeHtml(m.name||'')} ⬅️ ${window.escapeHtml(s.name||'')}</option>`).join('') : '';
              return `<optgroup label="${window.escapeHtml(m.name||'')}">${subOptions}</optgroup>`;
          }).join('');
      }
...
window.toggleMainLoc = function(mainId, e){ e.stopPropagation(); openMainLocIds[mainId] = !openMainLocIds[mainId]; window.renderLocationTree(); };
...
window.addMainLocation = async function(){ if(!isEditor) return; const input = document.getElementById('newMainLocInput'); if(!input.value.trim()) return; const newId = window.uid(); mainLocations.push({ id: newId, name: input.value.trim(), subLocations: [] }); openMainLocIds[newId] = true; input.value = ''; await window.saveMainLocationsToCloud(); };
window.deleteMainLocation = async function(mainId){ if(!isEditor || !confirm('حذف المنطقة؟')) return; const subIds = mainLocations.find(m => m.id === mainId)?.subLocations.map(s=>s.id) || []; mainLocations = mainLocations.filter(m => m.id !== mainId); const batch = db.batch(); compounds.filter(c => subIds.includes(c.locationId)).forEach(c => { batch.delete(db.collection('compounds').doc(c.id)); }); await batch.commit(); activeLocationIds = activeLocationIds.filter(id => id !== mainId && !subIds.includes(id)); await window.saveMainLocationsToCloud(); };
window.addSubLocation = async function(mainId){ if(!isEditor) return; const input = document.getElementById(`subInput_${mainId}`); if(!input || !input.value.trim()) return; mainLocations.find(m => m.id === mainId)?.subLocations.push({ id: window.uid(), name: input.value.trim() }); openMainLocIds[mainId] = true; await window.saveMainLocationsToCloud(); };
...
window.applyFilters = function(){ 
    ...
    filters = { 
        searchText: (window.getSafeVal('fSearchText') || '').trim().toLowerCase(), 
        minPrice: window.getRawNum(window.getSafeVal('fMinPrice')), 
        maxPrice: window.getRawNum(window.getSafeVal('fMaxPrice')), 
        downPaymentTarget: window.getRawNum(window.getSafeVal('fDownPayment')), 
        maxMonthlyInstallment: window.getRawNum(window.getSafeVal('fMonthlyInstallment')),
        ...
        sortOrder: window.getSafeVal('fSortOrder') || 'default'
    }; 
    window.renderGrid(); 
};
...
window.resetFilters = function(){ 
    ...
    window.setSafeVal('fMinPrice', ''); 
    window.setSafeVal('fMaxPrice', ''); 
    window.setSafeVal('fDownPayment', ''); 
    window.setSafeVal('fMonthlyInstallment', ''); 
    window.setSafeVal('fSortOrder', 'default');
...
window.findSubLocationName = function(subId){ 
...
window.generateDossierHTML = function(c) {
    const validPrices = (c.unitTypes||[]).map(u=>window.getRawNum(u.price)).filter(p => p !== null && !isNaN(p) && p > 0);
    ...
    if (c.projectType === 'commercial' && c.commercialPrices) {
        ...
        pDisplay = absoluteMin > 0 ? `يبدأ من ${window.formatNum(absoluteMin)}` : '-';
    } else { 
        if (c.pricePerMeterMin > 0 && c.pricePerMeterMax > 0 && c.pricePerMeterMin !== c.pricePerMeterMax) {
            pDisplay = `${window.formatNum(c.pricePerMeterMin)} - ${window.formatNum(c.pricePerMeterMax)}`;
        } else if (c.pricePerMeterMin > 0) {
            pDisplay = `يبدأ من ${window.formatNum(c.pricePerMeterMin)}`;
        } else if (c.pricePerMeter > 0) {
            pDisplay = window.formatNum(c.pricePerMeter);
        } else {
            pDisplay = '-';
        }
    }

    let phaseTag = c.phaseName ? `<span class="phase-tag">${window.escapeHtml(c.phaseName)}</span>` : '';
    ...
    return `<div class="dossier" onclick="openDetail('${c.id}')">
                ...
                <div class="dossier-company">${window.highlightText(c.companyName||'', filters.searchText)}</div>
                <div class="dossier-title">${window.highlightText(c.projectName||'بدون اسم', filters.searchText)} ${phaseTag}</div>
                <div class="dossier-badge">${PROJECT_TYPES[c.projectType||'residential']}</div>
                <div class="dossier-row"><b>الفرع:</b> <span>${window.highlightText(window.findSubLocationName(c.locationId), filters.searchText)}</span></div>
                <div class="dossier-row"><b>ارتفاع العمارات:</b> <span class="num">${c.floors ? window.escapeHtml(c.floors) : '-'}</span></div>
                ...
                <div class="dossier-meta">
                    ...
                    <div class="meta-chip"><span>أقل سعر وحدة</span><div>${minPrice ? window.formatNum(minPrice) : '-'}</div></div>
                </div>
            </div>`;
};
...
window.generateMasterDossierHTML = function(group) {
    let c = group[0]; 
    return `<div class="dossier master-dossier" onclick="openPhasesModal('${window.escapeHtml(String(c.projectName||'')).replace(/'/g, "\\'")}', '${window.escapeHtml(String(c.companyName||'')).replace(/'/g, "\\'")}')">
        <div class="master-badge">مراحل متعددة</div>
        <div class="dossier-company">${window.highlightText(c.companyName||'', filters.searchText)}</div>
        <div class="dossier-title">${window.highlightText(c.projectName||'بدون اسم', filters.searchText)}</div>
        <div class="dossier-badge">${PROJECT_TYPES[c.projectType||'residential']}</div>
        <div class="dossier-row"><b>الفرع:</b> <span>${window.highlightText(window.findSubLocationName(c.locationId), filters.searchText)}</span></div>
    ...
    try {
        ...
        let html = group.map(c => {
            return `<div class="detail-item" style="cursor:pointer; margin-bottom:0.625rem;" onclick="closeModal('phasesOverlay'); setTimeout(()=>openDetail('${c.id}'), 300)">
                <div style="color:var(--danger); font-size:1.125rem; font-weight:800; margin-bottom:0.3125rem;">${window.escapeHtml(c.phaseName || 'المرحلة الأساسية')}</div>
                <div style="font-size:0.75rem; color:var(--text-muted);">عمارات: <span class="num">${c.floors ? window.escapeHtml(c.floors) : '-'}</span> \vert{} تسليم: ${window.deliveryLabel(c.deliveryDate)}</div>
            </div>`;
        }).join('');
...
window.addCalcBulletRow = function(){ calcCustomBullets.push({id:uid(), type:'annual', percent:'', selectedYears:[]}); window.renderCalcBulletsRows(); };
...
window.renderCalcBulletsRows = function(){ 
    ...
    cbRows.innerHTML = calcCustomBullets.map(b=>`<div class="bullet-row" style="display:flex; gap:0.625rem; align-items:center; margin-bottom:0.625rem;">
        ...
        ${b.type==='annual'?`<div class="years-pills" style="margin-bottom:0.9375rem; display:flex; flex-wrap:wrap; gap:0.5rem; justify-content:center; width:100%;">${[1,2,3,4,5,6,7].map(yr=>`<div class="year-pill ${(b.selectedYears||[]).includes(yr)?'selected':''}" onclick="window.toggleYearSelection('${b.id}',${yr})">${yr}</div>`).join('')}</div>`:''}
    `).join(''); 
};
