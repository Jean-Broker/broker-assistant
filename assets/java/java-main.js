...
// ✨ دالة تحميل الداتا المضادة للانهيار 100% (بدون أي تخزين محلي يعلق الشاشة) ✨
...
function renderLocationTree(){
  try {
      ...
      console.error("Render Location Error:", e); 
      const wrap = document.getElementById('locationTree'); 
      if(wrap) wrap.innerHTML = `<div style="color:var(--danger); font-size:12px; text-align:center;">تعذر تحميل المناطق</div>`;
  }
}
...
function renderGrid(){
  try {
      ...
      const grid = document.getElementById('compoundGrid');
      if(grid) grid.innerHTML = `<div style="text-align:center; padding: 20px; color: var(--danger);">حدث خطأ أثناء عرض المشاريع. جاري المحاولة...</div>`;
  }
}
